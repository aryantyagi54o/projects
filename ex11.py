...
class GaitRecognitionApp(QWidget):
    ...
    def authenticate_user(self, live_landmarks):
        ...
        if not all_users:
            self.result_label.setText("No registered users in the database.")
            return

        min_distance = float("inf")
        matched_user = None

        for name, features_str in all_users:
            try:
                stored_features = eval(features_str)
                distance = self.euclidean_distance(live_features, stored_features)
                if distance < min_distance:
                    min_distance = distance
                    matched_user = name
            except Exception as e:
                print(f"Error comparing features for user {name}: {e}")

        self.result_label.setText(f"🔍 Closest match: {matched_user}\nDistance: {min_distance:.4f}")
